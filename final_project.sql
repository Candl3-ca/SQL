...
CREATE OR REPLACE PROCEDURE DELETE_TOURIST (VT_ID TOURIST.TOURIST_ID%TYPE)
AS
BEGIN
    IF(VT_ID IS NOT NULL) THEN
    DELETE FROM TOURIST WHERE TOURIST_ID = VT_ID;
    ELSE
        RAISE_APPLICATION_ERROR(-20001, 'TOURIST DOES NOT EXIST');
    END IF;
END;
...
FOR EACH ROW
DECLARE
VGUIDE_ID TRIP.GUIDES_ID%TYPE;
VTOURS_ID TRIP.TOURS_ID%TYPE;
BEGIN
